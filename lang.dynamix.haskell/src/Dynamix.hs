...
data Cmd val marks :: * -> * where
  ...
  -- control fragment
  Label     :: (Cont val marks -> Code val marks val) -> Cmd val marks val
  Label0    :: (Cont val marks -> Code val marks val) -> Cmd val marks val
...
label :: (Cont val marks -> Code val marks val) -> Code val marks val
label = liftF . Label

label0 :: (Cont val marks -> Code val marks val) -> Code val marks val
label0 = liftF . Label0
...
handler (Err m) _ =
  throwError m
handler (Label f) k = do
  p <- curPoint
  handle (do v <- f (p , k); k v)
handler (Label0 f) k = do
  p <- curPoint
  handle (f (p , k))
